...
import { Image as ImageIcon, Loader2, ArrowLeft, Save, AlertCircle } from 'lucide-react';
...
const EditBlog = () => {
  ...
  // Fetch existing blog and categories on load
  useEffect(() => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const msg = error.response?.data?.message || 'Failed to update blog post';
      toast.error(msg);
    } finally {
  ...
  if (isLoading) {
    return (
      <div className="flex flex-col items-center justify-center py-20 text-slate-400 space-y-4">
        ...
        <p className="text-sm font-medium tracking-wide">Loading blog post details...</p>
      </div>
    );
  }
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center space-x-2">
          ...
          <div>
            <h2 className="text-xl md:text-2xl font-bold font-display text-slate-100">Edit Blog Post</h2>
            <p className="text-slate-400 text-xs mt-0.5">Modify the title, content, or publication status.</p>
          </div>
        </div>
      </div>
                ...
                {imagePreview ? (
                  ...
                  <div className="relative rounded-xl overflow-hidden border border-slate-800 max-h-48 flex items-center justify-center bg-slate-900/50">
                    ...
                    <div 
                      ...
                      className="absolute inset-0 bg-slate-950/60 opacity-0 hover:opacity-100 transition duration-150 flex flex-col items-center justify-center text-xs font-semibold cursor-pointer"
                    >
